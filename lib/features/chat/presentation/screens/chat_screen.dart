...
//     _initializeChat(); // CHANGE THIS
//   }
//
//   // NEW METHOD: Load user ID first, then setup chat
...
//
//     // If that fails or returns null, try extracting from JWT
//     if (_currentUserId == null || _currentUserId!.isEmpty) {
//       _currentUserId = await TokenManager.instance.getUserIdFromToken();
//     }
//
//     log('👤 Current user ID: $_currentUserId', name: 'ChatScreen');
//
//     if (mounted) {
//       setState(() {
...
//
//     // Normalize both IDs (lowercase, trim whitespace)
//     final normalizedCurrent = _currentUserId!.toLowerCase().trim();
//     final normalizedSender = senderId.toLowerCase().trim();
//
//     final isMatch = normalizedCurrent == normalizedSender;
//
//     // Debug log
//     if (!isMatch) {
//       log('❌ ID Mismatch:', name: 'ChatScreen');
//       log('  Current: "$normalizedCurrent"', name: 'ChatScreen');
//       log('  Sender:  "$normalizedSender"', name: 'ChatScreen');
//     }
//
//     return isMatch;
//   }
//
//   // In _ChatScreenBodyState
//
...
//         // Listen for new messages (this should work without joining)
...
//         });
//
//         // Try to join conversation (but don't fail if it doesn't work)
...
//       } else {
//         log(
//           '⚠️ SignalR not available, messages will work via HTTP polling',
//           name: 'ChatScreen',
//         );
//       }
//     } catch (e) {
...
//       // Try to leave but don't wait for it
...
//
//     log('📤 Sending message from user: $_currentUserId', name: 'ChatScreen');
//     log('📤 Target conversation/user: $targetId', name: 'ChatScreen');
//     log('📤 Message content: "$content"', name: 'ChatScreen');
//
//     try {
...
//         // DETAILED LOGGING
//         log('✅ Message sent successfully!', name: 'ChatScreen');
//         log('✅ Message ID: ${state.message.id}', name: 'ChatScreen');
//         log(
//           '✅ Sender ID from response: "${state.message.senderId}"',
//           name: 'ChatScreen',
//         );
//         log('✅ Current user ID: "$_currentUserId"', name: 'ChatScreen');
//         log(
//           '✅ IDs match: ${state.message.senderId == _currentUserId}',
//           name: 'ChatScreen',
//         );
//         log('✅ Sender name: ${state.message.senderName}', name: 'ChatScreen');
//
//         // Check for whitespace issues
//         if (state.message.senderId.trim() != state.message.senderId) {
//           log(
//             '⚠️ WARNING: Sender ID has leading/trailing whitespace!',
//             name: 'ChatScreen',
//           );
//         }
//         if (_currentUserId != null &&
//             _currentUserId!.trim() != _currentUserId) {
//           log(
//             '⚠️ WARNING: Current user ID has leading/trailing whitespace!',
//             name: 'ChatScreen',
//           );
//         }
//
...
//           SnackBar(
//             content: Text('Failed to send message. The user may not exist.'),
...
//     // Show loading until user ID is loaded
//     if (!_isUserIdLoaded) {
//       return Scaffold(
...
//                     // DEBUG LOG
//                     log(
//                       '💬 Message from: ${message.senderId}, Current user: $_currentUserId, isMine: $isMine',
//                       name: 'ChatScreen',
//                     );
//
//                     final showDate =
...
// WORKAROUND SOLUTION: Use the senderId from your sent messages as the current user ID
